...
export default async function handler(req, res) {
  console.log('API handler: Request received for bowler by ID');
  ...
  try {
    ...
    // Get the bowler ID from the query parameters
    const { id } = req.query;
    
    if (!id) {
      console.error('API handler: No bowler ID provided');
    ...
    const apiUrl = `https://script.google.com/macros/s/${bowlerMacroId}/exec?action=getBowler&bowlerId=${id}`;
